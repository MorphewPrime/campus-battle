...
class Player extends PositionComponent with CollisionCallbacks {
  ...
  void update(double dt) {
    ...
    if ((position.y > screenD.y && isPointedUp) ||
        (position.y <= 0 && !isPointedUp)) {
      dir2 = Vector2(0, 0);
    } else if (isPointedUp) {
      dir2 = Vector2(0, -1);
    } else {
  ...
  void render(Canvas canvas) {
    ...
    if (isPointedUp) {
      ...
      canvas.rotate(5.49779);
    }

    canvas.drawOval(
        Rect.fromCenter(center: Offset(10, 0), width: width, height: height),
        _paint);
    // canvas.drawRect
    // Rect.fromCenter(
    //     center: size.toOffset(), width: size.x, height: size.y * 2),
    // _paint);
    canvas.restore();
  }
...
class Football extends FlameGame
  ...
  int enemyCount = 0;
  Football(BuildContext con, int i) {
    c = con;
    items = i;
  }
  @override
  ...
  //   player.isPointedUp = !player.isPointedUp;
  // }
...
class FootballW extends StatelessWidget {
  const FootballW({super.key});

  @override
  Widget build(BuildContext context) {
    runApp(GameWidget(game: Football(context, item)));
